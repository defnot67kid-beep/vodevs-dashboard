...
if not os.path.exists(USER_BG_FOLDER): os.makedirs(USER_BG_FOLDER)
if not os.path.exists(FONTS_FOLDER): os.makedirs(FONTS_FOLDER)
...
if os.path.exists(ADMIN_CONFIG_FILE):
    with open(ADMIN_CONFIG_FILE, 'r') as f: admin_config = json.load(f)
else:
    admin_config = {
        "default_font": "Inter", "default_font_size": 48,
        "default_bar_color": "#5865F2", "default_font_color": "#ffffff",
        "default_stats_color": "#b9bbbe", "default_opacity": 0
    }
...
@app.route('/')
def home(): return "✅ Rank Card Dashboard is online!"

@app.route('/dashboard')
def dashboard_redirect(): return redirect(url_for('login'))
...
def authorize():
    ...
    if not code: return "❌ No authorization code received.", 400
...
def dashboard(guild_id, user_id):
    ...
    if not font_files: font_files = ["Inter"]
...
def upload_bg(guild_id, user_id):
    if 'user_id' not in session or session['user_id'] != user_id: return "Unauthorized", 403
    if 'bg_image' not in request.files: return "No file", 400
...
def remove_bg(guild_id, user_id):
    ...
    if os.path.exists(filepath):
        try: os.remove(filepath); return "Removed", 200
        except: return "Failed to remove", 500
...
def get_card(guild_id, user_id):
    try:
        ...
        if os.path.exists(user_bg_path): bg_img = Image.open(user_bg_path).convert("RGB").resize((900, 250))
        elif os.path.exists(DEFAULT_BG_FILE): bg_img = Image.open(DEFAULT_BG_FILE).convert("RGB").resize((900, 250))
        else: bg_img = Image.new('RGB', (900, 250), color='#2f3136')
        ...
        if avatar_url:
            ...
            except Exception as e: print(f"⚠️ Avatar error: {e}")
...
def web_leaderboard(server_id):
    try:
        if not MONGO_URI: return "MongoDB URI not configured.", 500
        ...
        def get_level_from_xp(xp):
            level = 0
            while int(1000 * ((level + 1) ** 1.5)) <= xp: level += 1
            return level
        ...
        for doc in results:
            ...
            formatted_users.append({
                "username": username, "avatar_url": avatar_url, "level": level,
                "xp_formatted": xp_formatted, "messages": "0", "voice_time": "-"
            })
        if not has_data: return "No level data found for this server.", 404
...
def admin_panel():
    if 'admin_id' not in session: return redirect(url_for('admin_login_form'))
    global admin_config
    message = ""
    ...
    if request.method == 'POST':
        if 'bg_image' in request.files and request.files['bg_image'].filename != '':
            ...
            else: message = "❌ Invalid file type. Please upload a .ttf file."
            ...
            with open(ADMIN_CONFIG_FILE, 'w') as f: json.dump(admin_config, f, indent=4)
    ...
    <style>
        ...
        h1, h2 {{ color: #45ddc0; }} label {{ display: block; margin-top: 15px; font-weight: 600; color: #b9bbbe; }}
        ...
        input[type="color"] {{ height: 50px; padding: 0; cursor: pointer; }} input[type="number"] {{ width: 100px; }}
        button {{ background: #5865F2; border: none; font-weight: bold; cursor: pointer; transition: 0.2s; }} button:hover {{ background: #4752c4; }}
        .msg {{ color: #45ddc0; margin-top: 20px; text-align: center; font-weight: bold; }}
        .upload-zone {{ border: 2px dashed #40444b; padding: 30px; text-align: center; border-radius: 10px; margin-top: 10px; transition: all 0.3s; cursor: pointer; position: relative; }}
        .upload-zone.dragover {{ border-color: #5865F2; background: rgba(88, 101, 242, 0.1); transform: scale(1.02); }}
    ...
    <body>
        <div class="nav"><a href="/admin/logout">Logout</a></div>
        <h1>🛠️ Admin Control Panel</h1>
        <div class="msg">{message}</div>
        <div class="card">
            <h2>📷 Default Background</h2>
            <p style="color:#b9bbbe;">Upload a 900x250 background image.</p>
            <div class="upload-zone"><p>📁 Drag & Drop or Click to Upload Background</p>
                <form method="POST" enctype="multipart/form-data"><input type="file" name="bg_image" accept="image/png,image/jpeg"></form>
            </div>
        </div>
        <div class="card">
            <h2>📁 Font Manager (Drag & Drop)</h2>
            <p style="color:#b9bbbe;">Drag & Drop .ttf files here.</p>
            <div class="upload-zone" id="fontDropZone"><p>📁 Drag & Drop .ttf Fonts Here or Click</p>
                <form method="POST" enctype="multipart/form-data"><input type="file" name="font_file" accept=".ttf" id="fontInput"></form>
            </div>
            <div style="margin-top:15px; font-size:14px; color:#b9bbbe;"><strong>Installed Fonts:</strong> {', '.join(font_files) if font_files else 'None uploaded yet.'}</div>
        </div>
        <div class="card">
            <h2>⚙️ Global Default Settings</h2>
            <p style="color:#b9bbbe;">Settings apply to users who haven't customized.</p>
            <form method="POST">
                <input type="hidden" name="action" value="save_settings">
                <label>Default Font Family</label>
                <select name="default_font">{"".join([f'<option value="{f}" {"selected" if admin_config["default_font"] == f else ""}>{f}</option>' for f in font_files])}</select>
                <label>Default Font Size (px)</label><input type="number" name="default_font_size" value="{admin_config['default_font_size']}" min="20" max="100">
                <label>Default Username Color</label><input type="color" name="default_font_color" value="{admin_config['default_font_color']}">
                <label>Default Stats Color</label><input type="color" name="default_stats_color" value="{admin_config['default_stats_color']}">
                <label>Default Progress Bar Color</label><input type="color" name="default_bar_color" value="{admin_config['default_bar_color']}">
...
def admin_login_form():
    if 'admin_id' in session: return redirect(url_for('admin_panel'))
    return '''
    <!DOCTYPE html>
    <html><head><title>Admin Login</title><meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;700&display=swap" rel="stylesheet">
    <style>body{font-family:'Inter',sans-serif;background:#1e1e2e;color:#fff;display:flex;justify-content:center;align-items:center;height:100vh;margin:0}
    ...
    h1{color:#45ddc0;margin:0 0 10px 0;font-size:24px}p{color:#b9bbbe;margin:0 0 20px 0;font-size:14px}
    form{display:flex;flex-direction:column;gap:15px}input{padding:12px;border-radius:8px;border:1px solid #40444b;background:#202225;color:#fff;font-size:16px;outline:none}
    input:focus{border-color:#5865F2}button{background:#5865F2;color:#fff;border:none;padding:12px;border-radius:8px;font-weight:bold;font-size:16px;cursor:pointer}
    button:hover{background:#4752c4;transform:scale(1.02)}
    </style></head>
    <body><div class="card"><h1>🛡️ Admin Login</h1><p>Enter your admin credentials.</p>
    <form method="POST" action="/admin/login"><input type="text" name="username" placeholder="Admin Username" required><input type="password" name="password" placeholder="Admin Password" required><button type="submit">Login</button></form></div></body></html>
    '''
...
def admin_login_process():
    ...
    if not username or not password: return "❌ Username and password required.", 400
    admin = admins_collection.find_one({"username": username, "password": password})
    if not admin:
        return "<html><body style='background:#1e1e2e;color:#fff;display:flex;justify-content:center;align-items:center;height:100vh;'><div style='background:#2f3136;padding:40px;border-radius:16px;text-align:center;'><h1 style='color:#ff5555;'>❌ Invalid Credentials</h1><a href='/admin/login' style='color:#5865F2;'>Try again</a></div></body></html>", 401
...
def admin_signup(token):
    ...
    if not invite: return "❌ Invalid or already used invite link.", 404
    redirect_uri = url_for('admin_authorize', _external=True, _scheme='https')
    oauth_url = (f"https://discord.com/api/oauth2/authorize?client_id={CLIENT_ID}&redirect_uri={redirect_uri}&response_type=code&scope=identify&state={token}")
    return redirect(oauth_url)

@app.route('/admin/authorize')
def admin_authorize():
    code = request.args.get('code'); token = request.args.get('state')
    if not code or not token: return "❌ Missing authorization code or state.", 400
    invite = invites_collection.find_one({"token": token, "used": False})
    if not invite: return "❌ Invalid or expired invite token.", 404

    token_url = "https://discord.com/api/oauth2/token"
    data = {"client_id": CLIENT_ID, "client_secret": CLIENT_SECRET, "grant_type": "authorization_code", "code": code, "redirect_uri": "https://vodevs-dashboard-production.up.railway.app/admin/authorize"}
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    ...
    try:
        ...
        if discord_id != invite["discord_id"]: return "❌ This Discord account does not match the invite recipient.", 403
        invites_collection.update_one({"token": token}, {"$set": {"used": True}})

        return f"""
        <!DOCTYPE html><html><head><title>Create Admin Account</title><meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;700&display=swap" rel="stylesheet">
        <style>body{{font-family:'Inter',sans-serif;background:#1e1e2e;color:#fff;display:flex;justify-content:center;align-items:center;height:100vh;margin:0}}
        ...
        form{{display:flex;flex-direction:column;gap:15px}}input{{padding:12px;border-radius:8px;border:1px solid #40444b;background:#202225;color:#fff;font-size:16px;outline:none}}
        input:focus{{border-color:#5865F2}}button{{background:#5865F2;color:#fff;border:none;padding:12px;border-radius:8px;font-weight:bold;font-size:16px;cursor:pointer}}
        button:hover{{background:#4752c4;transform:scale(1.02)}}</style></head>
        <body><div class="card"><h1>🛡️ Create Admin Account</h1><p>Welcome {user_info['username']}! Set up your admin credentials.</p>
        <form method="POST" action="/admin/register/{discord_id}"><input type="text" name="username" placeholder="Admin Username" required><input type="password" name="password" placeholder="Admin Password" required><button type="submit">Create Account</button></form></div></body></html>
        """
    except Exception as e: return f"❌ Authentication failed: {e}", 500
...
def admin_register(discord_id):
    ...
    if existing: return "❌ This Discord account already has an admin account created.", 400
    result = admins_collection.insert_one({"discord_id": discord_id, "username": username, "password": password})
    session['admin_id'] = str(result.inserted_id); session['admin_username'] = username
    return redirect(url_for('admin_panel'))
...
def owner_panel(owner_token):
    ...
    if not secret or secret["token"] != owner_token: return "❌ Unauthorized. Invalid Owner Token.", 403
    ...
    # NOTE: Because the bot doesn't cache roles/channels into MongoDB, 
    # we serve placeholder data here for the UI to not crash. 
    # Add a role/channel cache in your bot to make this real-time.
